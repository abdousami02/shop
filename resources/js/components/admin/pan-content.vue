  ...
  <div class="panel-dash">
    <div class="panel-head">
      ...
      <ul class="links">
        ...
        <li class="link">
          <router-link to="?" class="active">All <span class="count">({{tb.total}})</span></router-link>
        </li>
        <slot name="link_top"></slot>
      </ul>
    </div>
    <!-- btn top & searchBar to edit or deleat multi -->
    <div class="panel-body" @click="click_row('out', $event)">
      <div class="edit">
        <div class="btns">
      ...
      <div class="panel-content">
        ...
        <table class="table table-bordered">
          ...
          <!-- table body -->
          <tbody>
            <tr class="elem-row" v-for="(row, index) in tb.data" :key="index" :data-id="row.id" @click="click_row(row.id,$event)" @dblclick="dblclick_row(row,$event)">
              <!-- <td class="t-0">
  ...
  methods: {
    ...
    click_row(id,event){
      ...
      if(event.target.tagName == "TD" || event.target.tagName == "P"){
        ...
        let row = $(event.target).parents('.panel-dash').find('.elem-row');
        row.siblings().removeClass('selected');
      }
    },
    dblclick_row(row, event){
