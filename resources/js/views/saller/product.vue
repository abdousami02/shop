...
export default {
  ...
  methods: {
    ...
    getData(page=1){
      let action = 'getData';
      axios.post("/api/saller/product?page="+page, {action: action}).then(response =>{
    ...
    getHelpInfo(){
      axios.post("/api/saller/product", {action: "getHelpInfo"}).then(response => {
    ...
    sendAction(id=''){
      if(this.action_wait == 0){      // wait for finish first response
        ...
        axios.post("/api/saller/product", data).then(response => {
    ...
    search_prod(str){
      ...
      axios.post("/api/saller/product", data).then(resp =>{
